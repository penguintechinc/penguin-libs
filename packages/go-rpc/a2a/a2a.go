...
// mounts both through Mount, preserving Mount's path and (lack of) auth
// contract — see Mount's doc comment.
//
// Because MountAgent builds the http.Handler internally, there is no
// handler argument left for a caller to pre-wrap with auth.HTTPMiddleware
// the way Mount's own usage sketch does. Callers needing auth on the
// JSON-RPC endpoint (i.e. every production deployment, per spec/SPEC.md §7)
// should call Mount directly with a2asrv.NewJSONRPCHandler(requestHandler,
// opts...) wrapped in auth.HTTPMiddleware, rather than MountAgent — wrapping
// mux as a whole after MountAgent returns is NOT equivalent, since it would
// also put the intentionally-public agent-card route behind auth.
