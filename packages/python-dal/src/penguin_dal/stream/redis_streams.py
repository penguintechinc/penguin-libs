...
class RedisStreamProducer:
    """Publishes to Redis Streams via XADD."""

    def __init__(self, config: RedisStreamConfig) -> None:
        """Initialize Redis Streams producer.

        Args:
            config: RedisStreamConfig instance.
    ...
    def flush(self, timeout: float = 10.0) -> None:
        """Flush pending messages (no-op for Redis).
...
class RedisStreamConsumer:
    """Consumes from Redis Streams via XREADGROUP."""
            ...
            for topic, entries in results:
                ...
                for msg_id, data in entries:
                    ...
                    stream_msg = StreamMessage(
                        topic=topic,
                        partition=0,  # Redis streams are single-partition per key
                        offset=msg_id,
                        key=key,
                        value=value,
                        headers=headers,
