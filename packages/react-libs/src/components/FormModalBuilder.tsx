...
/**
 * Logger utility for FormModalBuilder - sanitizes all sensitive data
 * Never logs: passwords, emails, personal data, field values
 * Only logs: field names, counts, structural info for debugging
 */
const log = {
  debug: (message: string, data?: Record<string, unknown>) => {
    if (process.env.NODE_ENV === 'development') {
      console.debug(`[FormModal] ${message}`, data ? sanitizeLogData(data) : '');
    }
  },
  info: (message: string, data?: Record<string, unknown>) => {
    console.info(`[FormModal] ${message}`, data ? sanitizeLogData(data) : '');
  },
  warn: (message: string, data?: Record<string, unknown>) => {
    console.warn(`[FormModal] ${message}`, data ? sanitizeLogData(data) : '');
  },
  error: (message: string, error?: unknown) => {
    // Only log error message, never full stack with potential sensitive context
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error(`[FormModal] ${message}`, errorMessage);
  },
};

function sanitizeLogData(data: Record<string, unknown>): Record<string, unknown> {
  const sanitized = { ...data };
  // Never log sensitive field types or their values
  const sensitiveKeys = [
    'password', 'email', 'token', 'secret', 'credential', 'value', 'formData',
    'data', 'values', 'defaultValue', 'currentValue'
  ];
  ...
  // Convert field arrays to just counts
  if (Array.isArray(sanitized.fields)) {
    sanitized.fieldCount = sanitized.fields.length;
    delete sanitized.fields;
  }
  if (Array.isArray(sanitized.errorFields)) {
    sanitized.errorFieldNames = (sanitized.errorFields as string[]);
  }
  return sanitized;
}
...
export const FormModalBuilder: React.FC<FormModalBuilderProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      log.info('Modal opened', {
        ...
        hasTabs: !!(manualTabs && manualTabs.length > 0),
        tabCount: manualTabs?.length ?? 0,
      });
    }
  ...
  const validate = useCallback((): boolean => {
    ...
    if (errorFieldNames.length > 0) {
      log.warn('Validation failed', {
        errorFieldNames,
        errorCount: errorFieldNames.length,
      });
    }
    return errorFieldNames.length === 0;
  }, [fields, formData]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    log.info('Form submission started', { title });
    ...
    if (!validate()) {
      ...
      if (tabs) {
        for (let i = 0; i < tabs.length; i++) {
          ...
          if (tabHasError) {
            setActiveTab(i);
            log.debug('Switched to tab with errors', { tabIndex: i });
            break;
          }
        }
      }
    ...
    try {
      ...
      log.info('Form submission successful', { title });
      onClose();
    } catch (error) {
  ...
  const handleNext = () => {
    if (tabs && activeTab < tabs.length - 1) {
      ...
      log.debug('Navigating to next tab', { from: activeTab, to: nextTab });
  ...
  const handlePrevious = () => {
    if (activeTab > 0) {
      ...
      log.debug('Navigating to previous tab', { from: activeTab, to: prevTab });
      setActiveTab(prevTab);
    }
  };

  const handleClose = useCallback(() => {
    log.info('Modal closed', { title });
    onClose();
  }, [onClose, title]);
