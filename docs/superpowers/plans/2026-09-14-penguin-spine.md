...
- **Shared-file collision (pre-flight review, session_01N2rQgkHY872RubwXoBZxtE):** despite the "only `packages/rust-spine/`" scope above, Task 21 appends to repo-root `Makefile` and creates a new, dedicated `.github/workflows/rust-spine.yml` (it does not touch `ci.yml`/`publish.yml`). `penguin-logging` (M1b), `penguin-bundle-host` (M1c) and `penguin-connectors` (M1d) also append to `Makefile`, each with distinct target names — a merge conflict there is textual (adjacent insertion), never semantic. Rebase onto the release branch immediately before opening the PR.
